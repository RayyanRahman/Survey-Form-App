...
const SurveyList = () => {
  ...
  const fetchSurveys = async () => {
    try {
      const response = await fetch('https://rayyan-survey-form.onrender.com/api/surveys');
  ...
  const handleDeleteSurvey = async (surveyId) => {
    try {
      const response = await fetch(`https://rayyan-survey-form.onrender.com/api/surveys/${surveyId}`, {
        method: 'DELETE',
      });
